...
#define SLEEP_MODE_TOTAL  21      // 수면모드 진행시간(A분 = B+C+D  수식에 맞게 설정할것)  defalut : 22 분
...
short user_fanSpeed = 115, user_Co2Concent = 255;
...
bool emergencyCheck();                    // 비상 상황 체크
void emergencyModeLogic();                // 비상 상황 로직
    ...
    for(unsigned int i=0;i<SLEEP_MODE_TOTAL*M;i++){  // 수면 시나리오    0.1초에 한번 루프 돌게.  25*M
      pastTime = millis();
        ...
        else if(i<(INIT_WIND_TIME+CO2_WIND_TIME)*M && i%10==0)   // FAN 최고 속도, CO2 분사 단계
        {
          ...
          if(emergencyCheck()){
            Serial.println(" └ 농도 200,000 ppm 초과 or 비정상 근접 : 비상모드 작동 "); 
            emergencyModeLogic();
            if (MODE == 1) break;   // 비상모드 5분 경과 -> 대기모드로 이동
          }
        }
...
/*-------------------------------------------------------------------------------------- 비상 상태 로직 */
bool emergencyCheck(){
  static int countCo2 = 0, countDist = 0;
  long co2 = co2sensing();
  
  if(co2>200000) countCo2++;
  else countCo2 = 0;

  if (getDistance() < DIST_LOWER) countDist++;
  else countDist = 0;

  //test log
  //Serial.print(" ");Serial.print(co2);Serial.print(" ");Serial.print(countDist);Serial.print(" ");
  
  if(countCo2 >= 5 || countDist >= 5)return true;
  else return false;
}

void emergencyModeLogic() {
    long pastTime, emerTime = 0;
    FAN(OFF); VALVE(OFF);
    MODE = 9;
    pixels.setPixelColor(0, pixels.Color(24, 0, 0));
    ...
    while (1) {
        ...
        while (millis() - pastTime < 1000)  // 루프주기 1초
            delay(1);
    }
